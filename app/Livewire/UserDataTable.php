...
final class UserDataTable extends PowerGridComponent
{
    public string $tableName = 'user-data-table-4najx6-table';
    ...
    public function deleteUser(int $user)
    {
        try {
            ...
            return redirect()->route('dashboard.master.user.index')->with('error', 'Gagal menghapus User: ' . $th->getMessage());
        }

        return redirect()->route('dashboard.master.user.index')->with('success', 'User berhasil dihapus.');
    }
    ...
    public function fields(): PowerGridFields
    {
        return PowerGrid::fields()
            ...
            ->add('name')
            ->add('email')
            ->add('created_at_formatted', fn (User $user) => Carbon::parse($user->created_at)->translatedFormat('j F Y H:i'));
    }
    ...
    public function columns(): array
    {
        return [
            ...
            Column::make('Waktu Input', 'created_at_formatted', 'created_at')
                ->sortable(),
    ...
    {
        $deletionRoute = route('dashboard.master.user.destroy', $row->id);
        ...
        return [
            Button::add('edit')
                ...
                ->route('dashboard.master.user.edit', ['user' => $row->id]),
            ...
            Button::add('delete')
                ...
                ->dispatch('delete-user', ['user' => $row->id]),
        ];
    }
