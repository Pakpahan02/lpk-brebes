...
final class BannerDataTable extends PowerGridComponent
{
    public string $tableName = 'banner-data-table-alsywb-table';
    ...
    public function fields(): PowerGridFields
    {
        ...
        ->add('title')
        ->add('description')
        ->add('image', fn (Banner $banner) => $banner->image
            ? '<img src="' . asset('storage/' . $banner->image) . '" class="h-24 rounded"/>'
            : '-')
        ->add('created_at_formatted', fn (Banner $banner) => Carbon::parse($banner->created_at)->translatedFormat('j F Y H:i'));
    }
    ...
    {
        $deletionRoute = route('dashboard.cms.banner.destroy', $row->id);
        ...
        return [
            Button::add('edit')
                ...
                ->route('dashboard.cms.banner.edit', ['banner' => $row->id]),
