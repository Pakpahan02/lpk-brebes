<x-layouts.front>
    <section class="container mx-auto px-6 py-10">
        ...
        <div class="mb-6 text-sm text-gray-500">
            <a href="{{ route('training.index') }}" class="hover:underline">Pelatihan</a>
            <span class="mx-2">/</span>
            <span>{{ Str::limit($training->title, 50) }}</span>
        </div>
            ...
            <div class="p-6 md:p-10 lg:p-12">
                ...
                <div class="flex items-center space-x-4 text-sm text-gray-500 mb-4">
                    <span class="inline-flex items-center">
                        ...
                        <span class="font-medium text-orange-600 uppercase">{{ $training->training_category->name }}</span>
                    </span>
                    <span class="inline-flex items-center">
                        ...
                        {{ $training->created_at->isoFormat('LL') }}
                    </span>
                </div>
                    ...
                    @foreach ($relatedTraining as $item)
                        <div class="bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:shadow-xl hover:-translate-y-1">
                            <a href="{{ route('training.show', $item->id) }}" class="block">
                                ...
                                {{-- Content --}}
                                <div class="p-4">
                                    <span class="text-xs font-medium text-orange-600 uppercase">{{ $item->training_category->name }}</span>
