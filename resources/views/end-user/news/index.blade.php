<x-layouts.front>
    <section class="container mx-auto px-6 py-10">
        <h2 class="text-center text-3xl font-bold mb-4 text-orange-600">Berita & Informasi</h2>
        <p class="text-center text-gray-600 mb-10">Temukan berita terkini dan informasi terbaru dari kami.</p>
        ...
        @if ($latestNews)
            <div class="mb-12">
                <div class="relative w-full h-[400px] md:h-[500px] lg:h-[600px] rounded-xl overflow-hidden shadow-lg">
                    {{-- Image --}}
                    @if ($latestNews->image)
                        <img src="{{ asset('storage/' . $latestNews->image) }}"
                    ...
                    <div class="absolute bottom-0 left-0 p-6 md:p-10 text-white z-10">
                        ...
                        <p class="mt-2 text-sm md:text-base line-clamp-3 md:line-clamp-4 max-w-2xl">
                            {{ Str::limit(strip_tags($latestNews->description), 200) }}
                        </p>
        ...
        <div class="mb-8 p-6 bg-gray-50 rounded-lg shadow-md">
            ...
            <form action="{{ route('news.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-4 gap-4 items-end">
                {{-- Filter Judul --}}
